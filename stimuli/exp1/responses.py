...
def learning_Response(win, thisTrial, this_respmap, modality):
    ...
    if this_respmap == 0: inst.text = "z = frecuente          espacio = débil          m = raro"
    else: inst.text = "z = raro         espacio = débil        m = frecuente"
    inst.draw()
    win.flip()          
    ...
    if this_respmap == 0: 
        ...
        if allKeys[0]=='z': 
            thisResp = "frecuente"
            if expect == "EXP" and thisTrial['catch'] == 0:
                outcome = 1
                fix_color = [-1,1,-1]
            else:
                outcome = 0
            ...
            thisResp = "raro"
            if expect == "VP" and thisTrial['catch'] == 0:
                outcome = 1
                fix_color = [-1,1,-1]
            else:
                outcome = 0
