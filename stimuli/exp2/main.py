...
#============================== DESIGN PRESETS ================================
#Blocks
learning_n = 4 #per modality, so it's x2
test_n = 10 # 5x2
blocks_explicit = [0,1]
 
mod = 1 # 0 = VISUAL / 1 = AUDITORY



blocks_learning = np.repeat(mod, learning_n)  
blocks_test = np.repeat(mod, test_n) 
np.random.shuffle(blocks_explicit)
...
#========================== EXPERIMENT STARTS =================================
if mod == 0:
    instr.main_instructions_v(win, basic_stim['grating_instr'], basic_stim['fixation_instr'], basic_stim['eye'], basic_stim['speaker_cross'])   
else:
    instr.main_instructions_a(win, basic_stim['grating_instr'], basic_stim['fixation_instr'], basic_stim['eye_cross'], basic_stim['speaker'])
...
for phase in [1,2]:
    if phase == 0: 
        ...
        if mod == 0:
            ...
            instr.training_learn(win, basic_stim, stim, mod, trial_list, resp_map) #5 training trials         
        else: 
            instr.learning_starts(win, basic_stim['grating'], basic_stim['fixation_point'], basic_stim['fixation_red']) # Learning phase instr
            instr.training_learn(win, basic_stim, stim, mod, trial_list, resp_map) # 5 training trials              
        ...
        # Initializing corresponding phase instructions
        if mod == 0: instr.test_starts_v(win)
        else: instr.test_starts_a(win)
        instr.training_test(win, basic_stim, stim, mod, trial_list, resp_map) # 2 training trials
        blocks = blocks_test
